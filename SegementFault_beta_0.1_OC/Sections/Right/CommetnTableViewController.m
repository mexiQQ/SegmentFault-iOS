...
- (void)getLatestLoans{
    [[CommentStore sharedStore] readNewData:^(NSMutableArray *array) {
        self.comments = array;
        self.myCommentDataSource = [[CommentDataSource alloc] initWithItems:array cellIdentifier:@"detailCommentCell" configureCellBlock:^(DetailCommentTableViewCell *cell, NSMutableDictionary *item) {
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section{
    DetailWriteCommentTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"detailWriteCommentCell"];
    
    // 给 label 增加点击事件
    UITapGestureRecognizer *tap=[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(wirteComment:)];
...
- (IBAction)wirteComment:(id)sender{
    ...
    [markdown setHandler:^(NSString *tagValue) {
        ...
        if([[NSUserDefaults standardUserDefaults] objectForKey:@"token"]){
            [[DetailQuestionStore sharedStore] commentAnswer:text answerId:self.id_ handle:^(NSDictionary *dic){

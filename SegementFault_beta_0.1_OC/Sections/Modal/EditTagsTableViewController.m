...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    EditTagTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"tagCell" forIndexPath:indexPath];
    if(cell == nil){
        cell = [[EditTagTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault
                                        reuseIdentifier:@"tagCell"];
    }
    cell.tagTitle.text = [self.tags objectAtIndex:indexPath.row];
    cell.tagImage.image = [UIImage imageNamed:[NSString stringWithFormat:@"%@.png",[self.tags objectAtIndex:indexPath.row]]];
    [cell.tagSwitch setTag:indexPath.row];
    [cell.tagSwitch addTarget:self action:@selector(switchAction:) forControlEvents:UIControlEventValueChanged];

...
- (UITableViewCell *)tableView:(UITableView *)tableView
  ...
  MeunTableViewCell *cell =
      [_mytableview dequeueReusableCellWithIdentifier:MenuCellIdentifier];
  if (cell == nil) {
  ...
  if (indexPath.section == 0 && indexPath.row == 0) {
    self.badge =
        [[LKBadgeView alloc] initWithFrame:CGRectMake(190, 13, 30, 28)];
    self.badge.text = @"";
  ...
  // 设置 cell 标题和图标
  NSString *title = [[_cellContent objectAtIndex:indexPath.section]
      objectAtIndex:indexPath.row];
  cell.menuTitle.text = title;
  cell.menuImage.image =
      [UIImage imageNamed:[NSString stringWithFormat:@"%@.png", title]];
  ...
  cell.selectedBackgroundView.backgroundColor =
      [[UIColor alloc] initWithRed:0 / 255.0f
                             green:154 / 255.0f
                              blue:97 / 255.0f
                             alpha:0.3];
  return cell;
...
- (UIView *)tableView:(UITableView *)tableView
  ...
  if (section == 0) {
    LeftTableViewUserSectionView *header =
        [[[NSBundle mainBundle] loadNibNamed:@"MenuTableViewUserSection"
                                       owner:self
    ...
    if (user) {
      ...
      NSString *avatarUrl = [user objectForKey:@"avatarUrl"];
      __block NSURL *url = [NSURL URLWithString:avatarUrl];
      dispatch_async(
      ...
      header.userResu.text =
          [NSString stringWithFormat:@"%@ 声望", [user objectForKey:@"rank"]];
    } else {
    ...
    LeftTableViewSectionView *header =
        [[[NSBundle mainBundle] loadNibNamed:@"MenuTableViewSectionView"
                                       owner:self
                                     options:nil] lastObject];
    header.sectionTitle.text = @"Tags";
    [header.addMenu addTarget:self
                       action:@selector(changeMenu)
...
- (void)tableView:(UITableView *)tableView
  ...
  if (indexPath.section == 0) {
    ...
    UITabBarController *mainViewController =
        [mainStoryboard instantiateViewControllerWithIdentifier:@"messagePage"];
    [self.mm_drawerController setCenterViewController:mainViewController
    ...
    UIStoryboard *mainStoryboard =
        [UIStoryboard storyboardWithName:@"Main" bundle:nil];
    UITabBarController *mainViewController =
        [mainStoryboard instantiateViewControllerWithIdentifier:@"mainPage"];
    [self.mm_drawerController setCenterViewController:mainViewController
                                   withCloseAnimation:YES
                                           completion:nil];
    [TagStore sharedStore].currentShowTag =
        [[_cellContent objectAtIndex:indexPath.section]
            objectAtIndex:indexPath.row];
  } else {
    ...
    case 0: {
      UIStoryboard *mainStoryboard =
          [UIStoryboard storyboardWithName:@"Main" bundle:nil];
      UITabBarController *mainViewController =
          [mainStoryboard instantiateViewControllerWithIdentifier:@"aboutPage"];
      [self.mm_drawerController setCenterViewController:mainViewController
                                     withCloseAnimation:YES
                                             completion:nil];
      break;
...
- (void)changeMenu {
  UIStoryboard *mainStoryboard =
      [UIStoryboard storyboardWithName:@"Main" bundle:nil];
  UIViewController *addMenuController =
      [mainStoryboard instantiateViewControllerWithIdentifier:@"addMenuPage"];
      ...
      dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSTimer *myTimer =
            [NSTimer scheduledTimerWithTimeInterval:10.0
                                             target:self
                                           selector:@selector(getMessageNumber:)
                                           userInfo:nil
                                            repeats:YES];
...
- (void)getMessageNumber:(NSNotification *)notification {
  if ([[NSUserDefaults standardUserDefaults] objectForKey:@"token"]) {
    NSString *url = [NSString
        stringWithFormat:@"http://api.segmentfault.com/user/stat?token=%@",

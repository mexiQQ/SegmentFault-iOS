...
- (void)setupView {
  ...
  [line setFrame:CGRectMake(2, self.userView.bounds.origin.y +
                                   self.userView.bounds.size.height / 2 + 1,
                            self.userView.bounds.size.width - 4, 1)];
...
- (IBAction)loginAction:(id)sender {
  ...
  STHTTPRequest *r = [STHTTPRequest
      requestWithURLString:@"http://api.segmentfault.com/user/login"];
  [r setPOSTDictionary:@{
    @"mail" : self.userEmail,
    @"password" : self.usrpassWord
  }];
  [r setCompletionJSONBlock:^(NSDictionary *header, NSDictionary *jsonObj) {
    ...
    if (status.integerValue == 0) {
      ...
      [[NSUserDefaults standardUserDefaults]
          setObject:[data objectForKey:@"user"]
             forKey:@"user"];
      [[NSUserDefaults standardUserDefaults]
          setObject:[data objectForKey:@"token"]
             forKey:@"token"];
...
- (IBAction)githubLoginAction:(id)sender {
  [[OAuthSignUtil sharedOAuthSignUtil] signInto:SignIntoGithub
                                 viewController:self];
  [OAuthSignUtil sharedOAuthSignUtil].oAuthDelegate = self;
}

- (IBAction)googleLoginAction:(id)sender {
  [[OAuthSignUtil sharedOAuthSignUtil] signInto:SignIntoGoogle
                                 viewController:self];
  [OAuthSignUtil sharedOAuthSignUtil].oAuthDelegate = self;
}
  ...
  [ShareSDK getUserInfoWithType:ShareTypeSinaWeibo
                    authOptions:authOptions
                         result:^(BOOL result, id<ISSPlatformUser> userInfo,
                           ...
                           if (result) {
                             id<ISSPlatformCredential> credential = [ShareSDK
                                 getCredentialWithType:ShareTypeSinaWeibo];
  ...
  [ShareSDK getUserInfoWithType:ShareTypeQQSpace
                    authOptions:authOptions
                         result:^(BOOL result, id<ISSPlatformUser> userInfo,
                           ...
                           if (result) {
                             id<ISSPlatformCredential> credential = [ShareSDK
                                 getCredentialWithType:ShareTypeQQSpace];
                             NSLog(@"token is %@", [credential token]);
...
- (IBAction)registerAction:(id)sender {
  [[UIApplication sharedApplication]
      openURL:[NSURL URLWithString:@"http://segmentfault.com/user/register"]];
}

- (IBAction)findPasswordAction:(id)sender {
  [[UIApplication sharedApplication]
      openURL:[NSURL URLWithString:@"http://segmentfault.com/user/forgot"]];
}

// MXOAurhUtil callback
- (void)didFinishOAuthSign:(NSString *)type
...
- (void)thirdLogin:(NSString *)access_Token type:(NSString *)type {
  ...
  STHTTPRequest *r = [STHTTPRequest
      requestWithURLString:@"http://api.segmentfault.com/user/oauth"];
  ...
  [r setCompletionJSONBlock:^(NSDictionary *header, NSDictionary *jsonObj) {
    ...
    if (status.integerValue == 0) {
      ...
      UINavigationController *bindNavigationController =
          [mainStoryboard instantiateViewControllerWithIdentifier:@"bind"];
      BindViewController *bindViewConttoller =
          bindNavigationController.childViewControllers[0];
      bindViewConttoller.accessToken = t;
      bindNavigationController.modalTransitionStyle =
          UIModalTransitionStyleFlipHorizontal;
      [self presentViewController:bindNavigationController
                         animated:YES
                       completion:nil];
    }
  }];

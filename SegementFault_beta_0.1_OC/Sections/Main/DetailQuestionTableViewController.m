...
@property(nonatomic, strong)
    DetailQuestionDataSource *myDetailQuestionDataSource;
...
- (void)viewDidLoad {
  ...
  [[NSNotificationCenter defaultCenter]
      addObserver:self
         selector:@selector(refreshTableViewHeight:)
             name:@"refreshQuestionHeight"
           object:nil];
    ...
    if (section != 0) {
      if (self.acceptAnswer == (id)[NSNull null]) {
        ...
        UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc]
            initWithTarget:self
                    action:@selector(showQuestionComment:)];
      ...
      DetailAnswerTableViewCell *cell =
          [tableView dequeueReusableCellWithIdentifier:@"detailAnswerCell"];
      if (section == 1) {
        [cell configureForCell:self.acceptAnswer
                         index:(NSInteger *)(section - 1)
                      accepted:YES];
        cell.commentLabel.tag = -1;
      } else {
        [cell configureForCell:self.availableAnswers[section - 2]
                         index:(NSInteger *)(section - 1)
                      accepted:NO];
  ...
  if (!self.question) {
    ...
    if (section == 0) {
      ...
      UIButton *a = [[UIButton alloc]
          initWithFrame:CGRectMake(self.view.bounds.size.width - 140, 10, 60,
                                   30)];
      ...
      UIButton *b = [[UIButton alloc]
          initWithFrame:CGRectMake(self.view.bounds.size.width - 70, 10, 60,
                                   30)];
...
- (void)actionSheet:(UIActionSheet *)actionSheet
  ...
  if (buttonIndex == 0) {
    //分享
    NSString *url =
        [NSString stringWithFormat:@"http://segmentfault.com%@",
                                   [self.question objectForKey:@"url"]];

    NSString *shareContent =
        [NSString stringWithFormat:
                      @"【%@】分享自 @SegmentFault，问题传送门：%@",
                      [self.question objectForKey:@"title"], url];

    UIImage *image = [UIImage imageNamed:@"sf.png"];
    //构造分享内容
    id<ISSContent> publishContent =
        [ShareSDK content:shareContent
    ...
    [container setIPadContainerWithView:actionSheet
...
- (IBAction)attentionAction:(id)sender {
  ...
  if ([[NSUserDefaults standardUserDefaults] objectForKey:@"token"]) {
    if (self.isFollowed.integerValue == 0) {
      [[DetailQuestionStore sharedStore]
          followQuestion:[self.question objectForKey:@"id"]
      ...
      [[DetailQuestionStore sharedStore]
          unfollowQuestion:[self.question objectForKey:@"id"]
                    handle:^(NSDictionary *dic) {
                      NSLog(@"%@", dic);
                      NSString *status = [dic objectForKey:@"status"];
                      if (status.integerValue == 0) {

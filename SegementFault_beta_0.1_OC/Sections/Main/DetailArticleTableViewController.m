      ...
      readNewData:^(NSDictionary *detailArticle, NSDictionary *detailComment) {
        ...
        self.myDetailArticleDataSource =
            [[DetailArticleDataSource alloc] initWithItems:self.articleDic
                ...
                cellIdentifier:@"detailArticleCell"
                aconfigureCellBlock:^(id cell, id item) {
  ...
  if (buttonIndex == 0) {
    ...
    NSString *url =
        [NSString stringWithFormat:@"http://segmentfault.com%@",
                                   [[self articleDic] objectForKey:@"editUrl"]];
    NSRange range = NSMakeRange(0, url.length - 5);
    url = [url substringWithRange:range];

    NSString *shareContent =
        [NSString stringWithFormat:
                      @"【%@】分享自 @SegmentFault，文章传送门：%@",
                      [[self articleDic] objectForKey:@"title"], url];
    ...
    MarkdownEditorViewController *markdown =
        markdownNav.childViewControllers[0];
...
- (IBAction)attentionAction:(id)sender {
  ...
  if ([[NSUserDefaults standardUserDefaults] objectForKey:@"token"]) {
    if (self.isLike.integerValue == 0) {
      [[DetailArticleStore sharedStore]
          likeArticle:[self.articleDic objectForKey:@"id"]
               handle:^(NSDictionary *dic) {
                 NSLog(@"%@", dic);
                 NSString *status = [dic objectForKey:@"status"];
      ...
      [[DetailArticleStore sharedStore]
          unlikeArticle:[self.articleDic objectForKey:@"id"]
                 handle:^(NSDictionary *dic) {
                   NSLog(@"%@", dic);
                   NSString *status = [dic objectForKey:@"status"];
                   if (status.integerValue == 0) {

...
- (void)readNewData:(void (^)(NSDictionary *, NSDictionary *))block {
        ...
        requestWithURL:
            [NSURL
                URLWithString:
                    [NSString
                        stringWithFormat:
                            @"http://api.segmentfault.com/question/%@?token=%@",
            ...
            [NSURL
                URLWithString:
                    [NSString stringWithFormat:
                                  @"http://api.segmentfault.com/answer/show/%@",
                                  [QuestionStore sharedStore]
                                      .currentShowQuestionId]]];
    [r startSynchronousWithError:&error];
    NSData *data = r.responseData;
    if (data) {
      detailAnwsers =
  ...
  stringWithFormat:@"http://api.segmentfault.com/question/%@",@"1010000002534328"];
...
- (void)hateQuestion:(NSString *)id_ handle:(void (^)(NSDictionary *dic))block {
  NSString *url = [NSString
      stringWithFormat:@"http://api.segmentfault.com/question/%@/hate?token=%@",
...
{
  NSString *url = [NSString
      stringWithFormat:@"http://api.segmentfault.com/question/%@/like?token=%@",
                       id_, [[NSUserDefaults standardUserDefaults]
                                objectForKey:@"token"]];
  STHTTPRequest *r = [STHTTPRequest requestWithURLString:url];
  r.POSTDictionary = @{
...
- (void)followQuestion:(NSString *)id_
  ...
  NSString *url = [NSString
      stringWithFormat:
          @"http://api.segmentfault.com/question/%@/follow?token=%@", id_,
...
- (void)unfollowQuestion:(NSString *)id_
  ...
  NSString *url = [NSString
      stringWithFormat:
          @"http://api.segmentfault.com/question/%@/unfollow?token=%@", id_,
          [[NSUserDefaults standardUserDefaults] objectForKey:@"token"]];
  STHTTPRequest *r = [STHTTPRequest requestWithURLString:url];
  r.POSTDictionary = @{
    @"token" : [[NSUserDefaults standardUserDefaults] objectForKey:@"token"]
...
- (void)hateAnswer:(NSString *)id_ handle:(void (^)(NSDictionary *dic))block {
  NSString *url = [NSString
      stringWithFormat:@"http://api.segmentfault.com/answer/%@/hate?token=%@",
                       id_, [[NSUserDefaults standardUserDefaults]
                                objectForKey:@"token"]];
  STHTTPRequest *r = [STHTTPRequest requestWithURLString:url];
  r.POSTDictionary = @{
...
- (void)likeAnswer:(NSString *)id_ handle:(void (^)(NSDictionary *dic))block {
  NSString *url = [NSString
      stringWithFormat:@"http://api.segmentfault.com/answer/%@/like?token=%@",
                       id_, [[NSUserDefaults standardUserDefaults]
                                objectForKey:@"token"]];
  STHTTPRequest *r = [STHTTPRequest requestWithURLString:url];
  r.POSTDictionary = @{
...
- (void)answerwQuestion:(NSString *)answerContent
                 handle:(void (^)(NSDictionary *dic))block {
  STHTTPRequest *r = [STHTTPRequest
      requestWithURLString:@"http://api.segmentfault.com/answer/post"];
  [r setPOSTDictionary:@{
    @"text" : answerContent,
    @"questionId" : [QuestionStore sharedStore].currentShowQuestionId,
    @"token" : [[NSUserDefaults standardUserDefaults] objectForKey:@"token"]
  }];
  [r setCompletionJSONBlock:^(NSDictionary *header, NSDictionary *jsonObj) {
    block(jsonObj);

...
- (void)webViewDidFinishLoad:(UIWebView *)webView {
  ...
  if (![DetailQuestionStore sharedStore].answersHeights) {
    [DetailQuestionStore sharedStore].answersHeights =
        [[NSMutableDictionary alloc] init];
  }

  if (![[DetailQuestionStore sharedStore].answersHeights
          objectForKey:self.indexTag] ||
      (documentHeight -
       ((NSNumber *)[[DetailQuestionStore sharedStore].answersHeights
            objectForKey:self.indexTag]).floatValue) > 1) {
    [[DetailQuestionStore sharedStore].answersHeights
        setObject:[NSNumber numberWithFloat:documentHeight]
           forKey:self.indexTag];
...
- (IBAction)liketap:(id)sender {
  if ([[NSUserDefaults standardUserDefaults] objectForKey:@"token"]) {
    [[DetailQuestionStore sharedStore]
        likeAnswer:self.answerModel.id_
...
- (IBAction)hatetap:(id)sender {
  if ([[NSUserDefaults standardUserDefaults] objectForKey:@"token"]) {
    [[DetailQuestionStore sharedStore]
        hateAnswer:self.answerModel.id_
            handle:^(NSDictionary *dic) {
              NSLog(@"%@", dic);
              NSString *status = [dic objectForKey:@"status"];
              if (status.integerValue == 0) {

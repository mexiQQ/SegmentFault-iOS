...
- (void)configureForCell:(NSDictionary *)item {
  ...
  [self.likeButton setTitle:self.questionModel.votes
                   forState:UIControlStateNormal];
  [self.contentWebView
      loadHTMLString:[[MXUtil sharedUtil]
                         formatHTMLFromMarkdown:self.questionModel.parsedText]
             baseURL:[[NSBundle mainBundle] bundleURL]];

  [self.likeButton addTarget:self
                      action:@selector(tapLike:)
            forControlEvents:UIControlEventTouchUpInside];
  [self.hateButton addTarget:self
                      action:@selector(tapHate:)
            forControlEvents:UIControlEventTouchUpInside];
}
...
- (void)webViewDidFinishLoad:(UIWebView *)webView {
  ...
  if (![DetailQuestionStore sharedStore].questionHeight ||
      (documentHeight -
       [DetailQuestionStore sharedStore].questionHeight.floatValue) > 1) {
    [DetailQuestionStore sharedStore].questionHeight =
        [NSNumber numberWithFloat:documentHeight];
    [[NSNotificationCenter defaultCenter]
        postNotificationName:@"refreshQuestionHeight"
                      object:self
                    userInfo:nil];
  }
}
...
- (IBAction)tapLike:(id)sender {
  if ([[NSUserDefaults standardUserDefaults] objectForKey:@"token"]) {
    [[DetailQuestionStore sharedStore]
        likeQuestion:self.questionModel.id_
              handle:^(NSDictionary *dic) {
                NSLog(@"%@", dic);
                NSString *status = [dic objectForKey:@"status"];
                if (status.integerValue == 0) {
                  [self.likeButton setTitle:[dic objectForKey:@"data"]
                                   forState:UIControlStateNormal];
                }
              }];
  } else {
...
- (IBAction)tapHate:(id)sender {
  if ([[NSUserDefaults standardUserDefaults] objectForKey:@"token"]) {
    [[DetailQuestionStore sharedStore]
        hateQuestion:self.questionModel.id_
              handle:^(NSDictionary *dic) {
                NSLog(@"%@", dic);
                NSString *status = [dic objectForKey:@"status"];
                if (status.integerValue == 0) {
